  ...
  xml.channel do
    ...
    @contents.all.each do |content|
      xml.item do
        ...
        xml.author 'DuoFold'
        xml.description render(partial: 'contents/content_body', formats: [:html], locals: {content: content}) + (content.content_picts.present? ?  raw('<hr>' + content.content_picts.map { |cp| image_tag(cp.pict.url(:thumbnail)) }.join) : '')
